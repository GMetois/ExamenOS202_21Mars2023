...
def save_as_md(cells, rank, symbols='⬜⬛',):
    res = np.empty(shape=cells.shape, dtype='<U')
    res[cells==0] = symbols[0]
    res[cells==1] = symbols[1]
    np.savetxt(f'resultat_{num_config:03d}_rank_{rank}.md', res, fmt='%s', delimiter='', header=f'Config #{num_config}', encoding='utf-8')
...
#Parallélisation statique sur la boucle des configurations
for num_config in range(rank, nombre_cas, size):
    t1 = time.time()
